...
class Cifar100:
    ...
    def __init__(self, args, training=True):
        if args.run_modelarts:
            ...
            #val_ir = os.path.join(args.data_url,'test')
            ...
            # data=next(self.train_dataset.create_dict_iterator())
            # for img in self.train_dataset:
            #     print(data)
            #if training:
...
def create_dataset_imagenet(dataset_dir, args, repeat_num=1, training=True,usage='Train'):
    ...
    #data = next(data_set.create_dict_iterator())
    #for image in data_set:
    #        print(data)
    print(data_set.get_dataset_size())
    #input=["image","fine_label","coarse_label"]
    #output=["image","label","coarse_label"]
...
#[0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010]
    ...
    #data_set = data_set.map(input_columns="coarse_label", num_parallel_workers=args.num_parallel_workers,
    # 
    #                        operations=transform_label)
    # data = next(data_set.create_dict_iterator())
    # for image in data_set:
